...
export function useAuraTranslator() {
  ...
  if (!translations?.value || !bannerTranslations?.value || !skillTranslations?.value) {
    throw new Error("Aura Translations we're not loaded");
  }

  function fixRePoETransaltionErrors(line: string): string {
    const fixedLines = line.replace("Your and nearby Allies'", 'You and nearby allies');
    ...
    aura.static.stats.forEach((stat: any) => {
      if (filterOutStats(stat)) {
        ...
        if (aura.stat_translation_file.includes('/banner_aura_skill')) {
          translation = unref(bannerTranslations).find((tr: any) => tr.ids.includes(stat.id));
        } else if (aura.stat_translation_file.includes('/aura_skill')) {
          translation = unref(translations).find((tr: any) => tr.ids.includes(stat.id));
        } else {
          translation = unref(skillTranslations).find((tr: any) => tr.ids.includes(stat.id));
        }
        if (!translation) {
          stats.push(stat.id);
        } else {
  ...
  const getQualityStat = (aura: any): string[][] => {
    ...
    aura.static.quality_stats.forEach((stat: any) => {
      ...
      if (!translation) {
        ...
        qualityStats[currentSet].push(fixRePoETransaltionErrors(translation.English[0].string));
      }
    });
...
export const isSecondaryValue = (stat: string): boolean => {
  ...
  if (stat === 'physical_damage_aura_nearby_enemies_physical_damage_taken_+%_max') {
    return true;
  }
