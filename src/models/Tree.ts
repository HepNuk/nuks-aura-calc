...
export default class Tree {
  ...
  public timelessJewel = 0;
  public clusterJewels = 0;
  public potency = false;
  ...
  public addNewNode(node: any) {
    ...
    if (this.treeClusters.has(treeNode.group)) {
      if (treeNode.isNotable) {
        this.treeClusters.get(treeNode.group)!.setNotable(treeNode);
      } else {
        this.treeClusters.get(treeNode.group)!.addSmallNode(treeNode);
      }
    } else {
  ...
  public getNotables(): TreeNode[] {
    ...
    this.treeClusters.forEach((treeCluster) => {
      if (treeCluster.notable) {
        notables.push(treeCluster.notable);
      }
    });
  ...
  public getAuraEffect(): number {
    ...
    if (this.potency) {
      ae += 3;
    } // FIXME: Make Constant or fetch data.
