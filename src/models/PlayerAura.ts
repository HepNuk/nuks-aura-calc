...
export default class PlayerAura {
  ...
  public getStatLines(
    auraEffect: number,
    supportGems: Map<string, SupportGem>
  ): string[] {
    if (this.level === 0) {
      return [];
    }
    ...
    this.auraDetails.stats.forEach((statLine, i) => {
      ...
      if (FILTER.youAndNearby.test(line) || FILTER.nearbyEnemies.test(line)) {
        if (this.auraDetails.statsPerLevel[i].value) {
          ...
          const statValue: number = this.scaleStatWithAuraEffect(
            this.auraDetails.statsPerLevel[i].value,
            ae,
            isIntegere
          );
          ...
          this.auraDetails.statsPerLevel[i][level - 1].forEach(
            (e: any, j: number) => {
              const isIntegere = !/Regenerate/.test(line);
              const statValue: number = this.scaleStatWithAuraEffect(
                e.value,
  ...
  public getQualityStatLines(
    auraEffect: number,
    supportGems: Map<string, SupportGem>
  ): string[] {
    if (this.level === 0 || this.altQuality === 0 || this.quality === 0) {
      return [];
    }
    ...
    this.auraDetails.qualityStats[aQual].forEach((qualityLine, i) => {
      if (
        FILTER.youAndNearby.test(qualityLine) ||
        FILTER.nearbyEnemies.test(qualityLine)
      ) {
        const isIntegere = !/Regenerate/.test(qualityLine);
        const statValue: number = this.scaleStatWithAuraEffect(
          this.auraDetails.statsPerQuality[aQual][i],
          ae,
          isIntegere,
          this.quality
        );
        const line = this.youAndNearbyAlliesQualityLine(
          qualityLine,
          statValue,
          i,
          aQual
  ...
  public auraEffectForGem(
    auraEffect: number,
    supportGems: Map<string, SupportGem>
  ): number {
    return (
      this.localAuraEffect +
      auraEffect +
      this.getGenerosityAuraEffect(supportGems)
    );
  }

  public aurasFromSkillsHelperArray(
    auraEffect: number,
    supportGems: Map<string, SupportGem>
  ): number {
    if (this.level === 0) {
  ...
  private getGenerosityAuraEffect(
    supportGems: Map<string, SupportGem>
  ): number {
    switch (this.generosityType) {
      case 1:
        if (this.generosityLevel > 40) {
          this.generosityLevel = 40;
        }
        if (this.generosityLevel <= 0) {
          break;
        }
        return (
          supportGems.get('generosity')?.statsPerLevel[
            this.generosityLevel - 1
          ] || 0
        );
      case 2:
        if (this.generosityLevel > 20) {
          this.generosityLevel = 20;
        }
        if (this.generosityLevel <= 0) {
          break;
        }
        return (
          supportGems.get('awakenedgenerosity')?.statsPerLevel[
            this.generosityLevel - 1
          ] || 0
        );
    }
  ...
  private handleNegativeValues(statIndex: number, line: string): string {
    ...
    if (line.slice(index, index + 'faster'.length + 5).includes('faster')) {
      ...
      line.slice(index, index + 'increased'.length + 5).includes('increased')
    ) {
  ...
  private filterYouAndNearbyAllies(line: string): string {
    ...
    if (filtered) {
      if (filtered[1].includes('to Accuracy Rating')) {
        return filtered[1].replace('to', 'additional');
      } // Fix for precision.
      return filtered[1];
    }
    return line;
  ...
  private youAndNearbyAlliesStatLine(
    statLine: string,
    statValue: number,
    i: number,
    j = 0
    ...
    if (statValue < 0) {
      statLine = this.handleNegativeValues(i, statLine);
    }
    ...
    if (statValue > 0) {
      return statLine.replace('{j}', statValue.toString());
    }
    return null;
  }

  private youAndNearbyAlliesQualityLine(
    qualityLine: string,
    statValue: number,
    i: number,
    aQual: number = null
    ...
    if (statValue < 0) {
      line = this.handleNegativeValues(0, line);
    }
    statValue = Math.abs(statValue);
    if (statValue > 0) {
      return line.replace('{0}', statValue.toString());
    }
    return null;
  }

  private scaleStatWithAuraEffect(
    statValue: number,
    ae: number,
    integere: boolean,
    quality: number = null
