...
const STAT_REGEX = {
  ...
  aurasFromSkill:
    /Auras from your Skills grant \+?([0-9.]*)% [a-zA-Z ]*you and Allies/,
  takeIncreasedDamage: /take (\d*)% increased Damage/,
  grantsXOfReservation:
    /Grants [a-zA-Z ]*(\d*)% of your [a-zA-z ]* and nearby Allies/,
...
export default class AscNode extends AbsrtactNode {
  ...
  private setBooleans(
    isAurasFromSkill = false,
    isAuraEffect = false,
    scalesWithReservation: string = null
  ): void {
  ...
  private setupAurasFromSkill(stat: string) {
    ...
    const regex = stat.match(
      /Auras from your Skills grant (\+?)(?:[0-9.]*)*([a-zA-Z %]*) to you and Allies/
    );
